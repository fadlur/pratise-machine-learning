...
outliers_price = (df['price'] < lower_bound) | (df['price'] > upper_bound)
print(f"\n[2c] Outlier harga (IQR method): {outliers_price.sum()} baris")
print(f"     Range valid: {lower_bound:,.0f} - {upper_bound:,.0f}")

# Ganti outlier dengan NaN (nanti akan di-handle di step imputasi)
...
# Masalah: "Budi Santoso", "budi santoso", "BUDI SANTOSO" = orang sama
...
print(f"\n[2e] Customer unik setelah cleaning: {df['customer_name'].nunique()}")
print(df['customer_name'].value_counts().head(10))


# ----- STEP 2f: Standardisasi Kolom `city` -----
# Masalah: "Jakarta", "jakarta", "JAKARTA", "Jkt" = kota sama
# Strategi: lowercase dulu, lalu mapping singkatan → nama lengkap

df['city'] = df['city'].str.strip().str.lower()

# Mapping singkatan dan variasi ke nama standar
city_mapping = {
    'jakarta': 'Jakarta',
    'jkt': 'Jakarta',
    'surabaya': 'Surabaya',
...
# Masalah: inkonsistensi huruf besar/kecil + typo
...
category_mapping = {
    ...
    'eletronik': 'Elektronik',      # fix typo
    'fashion': 'Fashion',
    'fshion': 'Fashion',            # fix typo
    'makanan': 'Makanan',
    'makan': 'Makanan',             # fix singkatan
    'kesehatan': 'Kesehatan',
}
...
# Customer name → hapus baris yang kosong (kita butuh tahu siapa customer-nya)
